...
class InputMethodLineEdit(QLineEdit):
    """专门优化输入法支持的QLineEdit"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("色弱图谱识别程序 - 输入法修复版")
    ...
    def _setup_ui(self):
        ...
        # 使用优化的输入框
        self.answer_input = InputMethodLineEdit()
        ...
        self.status_bar.showMessage("就绪 - 请测试中文输入")
        ...
        if CONFIG_FILE.exists():
            try:
                ...
                if hasattr(self, 'image_list') and self.image_list:
                    last_index = config.get('last_image_index', 0)
                    if 0 <= last_index < len(self.image_list):
